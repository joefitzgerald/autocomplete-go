...
class GocodeProvider {
  constructor (goconfigFunc, gogetFunc) {
    ...
    this.completionMode = atom.config.get('autocomplete-go.completionMode')
  ...
  generateSnippet (name, match) {
    ...
    if (args.length === 1) {
      let vv = args[0]
      let hasDots = false
      if (this.completionMode === 'Arg name') {
        let parts = args[0].split(' ')
        vv = parts[0]
        hasDots = parts[1].startsWith('...')
      }
      signature = {
        snippet: name + '(${1:' + vv + '}',
        displayText: name + '(' + args[0] + ')'
      }
      if (hasDots) {
        signature.snippet = signature.snippet + '${2:...}'
      }
      signature.snippet = signature.snippet + ')'
      if (this.completionMode === 'None') {
    ...
    for (let arg of args) {
      ...
      if (i === 1) {
        signature.snippet = name + '(${' + i + ':' + vv + '}'
        signature.displayText = name + '(' + arg
      } else {
        signature.snippet = signature.snippet + ', ${' + i + ':' + vv + '}'
        signature.displayText = signature.displayText + ', ' + arg
      }
      i = i + 1
      if (hasDots) {
        signature.snippet = signature.snippet + '${' + i + ':...}'
        i = i + 1
      }
    }
